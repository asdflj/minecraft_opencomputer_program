...
uninit() 卸载幕布，清理内存
drawline(x_min,x_max,y,background_color,foreground_color,text,run)画一条线 x_min,x_max 为X轴的最小和最大值，
y=y轴 background_color=背景颜色 foreground_color=字体颜色 text=文字 run=函数 
fill_background_color(background_color)填充所有背景颜色 background_color=背景颜色
fill_foreground_color(foreground_color)填充所有字体颜色 foreground_color=字体颜色
clear_text() 清除所有文字
setresolution(x,y) 设置显示大小 
draw_simple(x,y,background_color,foreground_color,text,run) 设置单个位置 text只能为单英文或者数字
...
draw_simple(x_min,x_max,y,background_color,foreground_color,text,run)
